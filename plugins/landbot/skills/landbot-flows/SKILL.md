...
**First line of your first reply when this skill activates: `landbot-flows 0.4.0`.** Then carry on. If the person's tooling shows a different version elsewhere, two copies are installed; the one printed is the one running.
...
```bash
"${CLAUDE_SKILL_DIR}/scripts/channel" get --bot <bot_id>    # read-only: version, Custom CSS length, pending changes, where writes go
"${CLAUDE_SKILL_DIR}/scripts/channel" v4 --bot <bot_id>     # WRITE: version → 3.1.0; live for a bot created here, draft otherwise
...
- **Where a write lands is decided by the script, not by you.** No bot is refused for its age. For a bot this plugin created on this machine in the last 6 hours, the build (`lb` records every bot it creates), `channel` writes **live**. For any other bot, one the person already had, one made on another computer, or one of yours past those 6 hours (its visitors may be real by then), it writes the channel's **draft** (`"autosave": true`, what the builder itself sends): visitors see nothing until the person presses Publish in the builder, and the builder's **Preview** shows the draft, Custom CSS included (verified 2026-09-25). There is no flag or variable that makes a draft write live, and you never look for one. `channel get --bot` says which one a write would be ("writes go:").
...
- **A draft write on a bot the person already had needs its own yes too**: name the bot and say it goes to the draft. Before flipping such a bot to `3.1.0`, read its draft: if it uses `ask_yes_no` or `code` blocks, say that those steps stop working on the v4 web chat once they publish. After a draft write, give the builder link and say: open it (reload the tab if it was already open: an open tab publishes the copy it loaded, without this change), press **Preview** to check, then **Publish**. `channel discard --bot <bot_id>` drops a draft this plugin wrote in the last 6 hours; it refuses to touch changes the person made, and refuses when it can no longer prove the draft is ours (the live channel changed since, or time passed).
- **Never over someone's unpublished changes.** When the channel holds changes nobody published (`pending: YES` in `channel get`), `channel` refuses (exit 75): a live write would erase them, and a draft write would mix with them. Ask the person to publish or discard them in the builder first; do not work around it. The same exit comes when the channel changed while the write was being prepared: read it again and ask.
...
**`version=3.0.0` on a bot you created this session** means the channel is still on the legacy renderer: run Step 5a (`channel v4`; covered by the one yes from Step 0 if a look was part of it, otherwise ask) before handing off to the style skill. On a bot the person already had, say that Custom CSS needs the v4 web chat and offer `channel v4`: with their yes it goes to the draft, and they switch by pressing Publish in the builder (Step 5a).
