...
- **An open builder tab undoes your writes.** The builder keeps its own copy of the flow, never reloads it, and its next save (any edit the person makes, even adding one block) writes that copy back over everything you wrote through the API, with no violation and **no change to the bot's `edited_at`**. Verified on production 2026-09-22. So: before the first write to a bot, ask the person to close that bot in the builder until you hand it back; before a publish, re-read the draft and compare it with what you wrote (block ids and each block's params), and if anything you wrote is gone, stop and say so instead of publishing.
