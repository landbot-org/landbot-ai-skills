...
- Every API request from `lb` carries a user agent such as `landbot-plugin/0.3.2 (landbot-flows; agent=claude-code)`: the plugin version and which coding agent ran it, so Landbot can count plugin use and failures per version. No user, machine or content data is added.
