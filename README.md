...
| `landbot-style` | Turns "make it look like our site / dark / like a form" into one Custom CSS block, pushes it to the channel (or tells you where to paste it) and verifies it on the published share URL. |
...
1. **Your account is not enabled for the Bots API yet.** The API is in preview and Landbot switches it on per account. Open an issue here with your account email (never the token), or use the assistant on the skills page, and we enable it. Nothing to fix on your side.
